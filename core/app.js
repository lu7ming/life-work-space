  ...
  async function loadModule(routeName, params = {}) {
    ...
    try {
      ...
      if (moduleObj && moduleObj.init) {
        ...
        // 暴露 DashboardModule 到全局，供顶栏生命树等按钮调用
        if (routeName === 'dashboard') {
          window.DashboardModule = moduleObj;
        }
      }
  ...
  function initTopbar() {
    ...
    document.querySelectorAll('.topbar-icon-btn:not(#topbar-more-btn):not(#notif-bell):not(#topbar-travel-btn):not(#topbar-refresh-btn)').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        if (tip.includes('生命树')) {
          // 生命树按钮：如果在 Dashboard 页面，直接打开全屏生命树；否则先跳转到 Dashboard 再打开
          const currentRoute = Router.getCurrentRoute?.() || 'dashboard';
          if (currentRoute === 'dashboard' && window.DashboardModule?.openLifeTreeFullView) {
            window.DashboardModule.openLifeTreeFullView();
          } else {
            Router.navigate('dashboard');
            // 延迟打开，等 Dashboard 加载完成
            setTimeout(() => {
              if (window.DashboardModule?.openLifeTreeFullView) {
                window.DashboardModule.openLifeTreeFullView();
              }
            }, 500);
          }
