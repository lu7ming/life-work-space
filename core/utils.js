...
/**
 * SharedKnowledge - AI 知识共享层
 */
const SharedKnowledge = (() => {
  let _knowledge = {};

  function set(key, value) {
    _knowledge[key] = { value, source: 'xiaolu', updatedAt: Date.now() };
    _persist();
  }

  function get(key) {
    return _knowledge[key]?.value;
  }

  function setAnalysis(key, analysis) {
    _knowledge[key] = { value: analysis, source: 'nicole', updatedAt: Date.now() };
    _persist();
  }
  ...
  function getContextForPrompt(aiType) {
    ...
    const relevant = entries
      .filter(([k, v]) => Date.now() - v.updatedAt < 86400000)
  ...
  async function _persist() {
    try {
      await Storage.put('settings', { key: 'shared_knowledge', value: _knowledge });
    } catch (e) { /* 静默 */ }
  }

  async function load() {
    try {
      const setting = await Storage.get('settings', 'shared_knowledge');
      if (setting) _knowledge = setting.value || {};
    } catch (e) { /* 静默 */ }
  }

  return { set, get, setAnalysis, getContextForPrompt, load };
})();

/**
 * AIOrchestrator - AI 智能路由与协作
 */
const AIOrchestrator = (() => {
  function route(userMessage) {
    if (/分析|检查|审计|报告|洞察|效率|健康|总结|周报|月报/.test(userMessage)) return 'nicole';
    if (/记录|创建|打卡|花了|任务|聊天|帮忙|提醒/.test(userMessage)) return 'xiaolu';
    return 'xiaolu';
  }

  async function handle(userMessage) {
    const target = route(userMessage);
    if (target === 'nicole' && typeof NicoleModule !== 'undefined') {
      return { ai: 'nicole', action: () => NicoleModule.open() };
    }
    if (typeof XiaoluModule !== 'undefined') {
      return { ai: 'xiaolu', action: () => XiaoluModule.open() };
    }
    return null;
  }
  ...
  async function notifyAction(tool, result) {
    if (result.success) {
      ...
      try { localStorage.removeItem(`nicole_daily_insight_${today}`); } catch (e) {}
    }
  }

  return { route, handle, notifyAction };
})();
