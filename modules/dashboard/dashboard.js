...
export const DashboardModule = (() => {
    ...
    try {
      switch (widget.type) {
        ...
        case 'achievements-widget':
          await renderAchievementsWidget(container, widget.config);
          break;
        case 'lifetree-widget':
          await renderLifeTreeWidget(container, widget.config);
          break;
        default:
  ...
  async function renderAchievementsWidget(container, config) {
    try {
      ...
      if (expandBtn) {
        expandBtn.addEventListener('click', () => {
          openAchievementsFullView();
        });
      }
    } catch (e) {
      console.warn('[Dashboard] 成就Widget渲染失败:', e);
      container.innerHTML = '<div class="dash-widget-no-data">加载失败</div>';
    }
  }
  ...
  async function renderLifeTreeWidget(container, config) {
    try {
      container.innerHTML = `
        <div class="dash-widget-lifetree-container">
          <div class="dash-widget-lifetree-label">🌳 我的生命树</div>
          <div class="dash-widget-lifetree-mini" id="dash-lifetree-mini">
            <svg viewBox="0 0 200 200" class="dash-widget-lifetree-svg">
              <!-- 树干 -->
              <rect x="95" y="130" width="10" height="50" fill="#8B6F47" rx="2"/>
              <!-- 树冠 -->
              <circle cx="100" cy="90" r="55" fill="#7EBF8E" opacity="0.9"/>
              <circle cx="75" cy="100" r="35" fill="#6BAE7D" opacity="0.8"/>
              <circle cx="125" cy="100" r="35" fill="#6BAE7D" opacity="0.8"/>
              <circle cx="100" cy="60" r="30" fill="#8FC99A" opacity="0.9"/>
              <!-- 果实/装饰 -->
              <circle cx="80" cy="85" r="4" fill="#E8A87C"/>
              <circle cx="115" cy="75" r="3" fill="#E8A87C"/>
              <circle cx="95" cy="105" r="3" fill="#E8A87C"/>
            </svg>
          </div>
          <button class="dash-widget-lifetree-expand" id="dash-lifetree-expand-btn">查看完整生命树 →</button>
        </div>
      `;

      // 绑定展开按钮
      const expandBtn = document.getElementById('dash-lifetree-expand-btn');
      if (expandBtn) {
        expandBtn.addEventListener('click', () => {
          openLifeTreeFullView();
        });
      }
    } catch (e) {
      console.warn('[Dashboard] 生命树Widget渲染失败:', e);
      container.innerHTML = '<div class="dash-widget-no-data">加载失败</div>';
    }
  }
...
const CalendarSub = (() => {
  ...
  // ===== 初始化 =====
  async function init() {
    console.log('[Dashboard/Calendar] 日历子模块初始化...');
    clearCache();
    bindEvents();
  ...
  function bindEvents() {
    const prevBtn = document.getElementById('dash-cal-prev-month');
    const nextBtn = document.getElementById('dash-cal-next-month');
    const todayBtn = document.getElementById('dash-cal-today-btn');
    const closeBtn = document.getElementById('dash-cal-detail-close');
    const overlay = document.getElementById('dash-cal-detail-overlay');
  ...
  async function renderCalendar() {
    ...
    const daysEl = document.getElementById('dash-cal-days');
  ...
  async function openDetail(dateStr) {
    const panel = document.getElementById('dash-cal-detail-panel');
    const overlay = document.getElementById('dash-cal-detail-overlay');
    const dateLabel = document.getElementById('dash-cal-detail-date');
    const body = document.getElementById('dash-cal-detail-body');
  ...
  function destroy() {
    ...
    console.log('[Dashboard/Calendar] 子模块已销毁');
  }
  ...
  // Update destroy to clean up calendar
  ...
  // ====================================================
  //  成就子模块（原 achievements 模块，迁移至 Dashboard）
  // ====================================================
/**
...
const AchievementsFullSub = (() => {
  ...
  return { init, destroy, openFullView, closeFullView };
})();


  // ====================================================
  //  生命树子模块（原 lifetree 模块，迁移至 Dashboard）
  // ====================================================
/**
 * 生命树模块 - 人生工作台
...
const LifeTreeFullSub = (() => {
  ...
  function destroy() {
    ...
    _intervals.forEach(id => clearInterval(id));
    _intervals = [];
    console.log('[Dashboard/LifeTree] 子模块已销毁');
  }
  ...
  function openFullView() {
    const overlay = document.getElementById('dash-lt-full-overlay');
    if (overlay) {
      overlay.style.display = 'flex';
      overlay.classList.add('show');
      // 绑定关闭按钮
      const closeBtn = document.getElementById('dash-lt-full-close-btn');
      if (closeBtn && !closeBtn._bound) {
        closeBtn.addEventListener('click', closeFullView);
        closeBtn._bound = true;
      }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function openAchievementsFullView() {
    AchievementsFullSub.openFullView();
  }

  function closeAchievementsFullView() {
    AchievementsFullSub.closeFullView();
  }

  // ===== 生命树全屏视图 =====
  function openLifeTreeFullView() {
    LifeTreeFullSub.openFullView();
  }

  function closeLifeTreeFullView() {
    LifeTreeFullSub.closeFullView();
  }

  function destroyAchievementsLT() {
    try { AchievementsFullSub.destroy(); } catch(e) {}
    try { LifeTreeFullSub.destroy(); } catch(e) {}
  }
